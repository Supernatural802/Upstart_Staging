(function() {
    ...
    t.set(blueWipe, {x:670});
    t.set(ladderHide, {x:-320});
    t.set(cta, { transformOrigin: "70% 100%" });
    //t.set(copy02, {x:728});

       t.set([copy01, copy02], setValues); //sets all graphic elements opacity on stage to 0
    ...
    //intro frame 01
        ...
        t.to(blueWipe, 1, {x:"-=530", ease: Power3.easeOut, delay:4});

        t.to(copy02, 1, { autoAlpha:1, ease: Power3.easeOut, delay:5});
        t.to(copy01, .5, {autoAlpha:0, ease: Power3.easeInOut, delay:4})

        t.to(blueWipe, .5, { x:355, ease: Power2.easeInOut, delay:7});

        t.to(container, .5, { x:728, ease: Power2.easeInOut, delay:7});
